    ...
    I::Item: Into<String>,
{
    ...
    match args[1].as_str() {
        "-h" | "--help" | "help" => {
            println!("{usage}");
            println!("examples:");
            println!("  codex-session-tui copy <session-id> user@remote:/home/user/data/cases");
            ...
            println!("  codex-session-tui ls user@remote:/home/user/data/cases");
            println!("  codex-session-tui repair-index user@remote");
            std::process::exit(0);
        }
...
mod tests {
    ...
    fn poll_startup_load_applies_partial_remote_snapshot_without_finishing_loader() {
        ...
        let remote = ProjectBucket {
            ...
            machine_codex_home: Some(String::from("/home/user/.codex")),
            machine_exec_prefix: None,
    ...
    fn parse_cli_command_parses_copy() {
        let cmd = parse_cli_command([
            ...
            "copy",
            "session-123",
            "user@remote:/home/user/data/cases",
        ])
        .expect("parse");
        assert_eq!(
            cmd,
            Some(CliCommand::Copy {
                ...
                target: String::from("user@remote:/home/user/data/cases"),
            })
        );
    }
    ...
    fn parse_cli_command_parses_tree_and_ls() {
        ...
        assert_eq!(
            parse_cli_command(["codex-session-tui", "ls", "user@remote:/home/user/data/cases"])
                .expect("parse"),
            Some(CliCommand::Ls {
                target: Some(String::from("user@remote:/home/user/data/cases")),
            })
        );
    }
    ...
    fn parse_cli_command_parses_repair_index() {
        ...
        assert_eq!(
            parse_cli_command(["codex-session-tui", "repair-index", "user@remote"]).expect("parse"),
            Some(CliCommand::RepairIndex {
                target: Some(String::from("user@remote")),
            })
        );
    }
    ...
    fn cli_ls_lines_follow_browser_tree_model() {
        ...
        app.projects = vec![ProjectBucket {
            machine_name: String::from("user@remote"),
            machine_target: Some(String::from("pi@192.0.2.124")),
            machine_codex_home: Some(String::from("/home/user/.codex")),
            machine_exec_prefix: None,
            cwd: String::from("/home/user/data/cases"),
            sessions: vec![sample_session(
                "/tmp/s1.jsonl",
                "/home/user/data/cases",
                "abc123456789",
            )],
        }];
        app.all_projects = app.projects.clone();
        ...
        assert!(roots.iter().any(|line| line.contains("user@remote")));

        let machine = app.cli_ls_lines(Some("user@remote")).expect("ls machine");
        assert!(
            machine
                .iter()
                .any(|line| line.contains("home/user/data/cases"))
        );

        let folder = app
            .cli_ls_lines(Some("user@remote:/home/user/data/cases"))
            .expect("ls folder");
    ...
    fn cli_tree_lines_include_nested_rows() {
        ...
        app.projects = vec![ProjectBucket {
            ...
            machine_codex_home: None,
            machine_exec_prefix: None,
            cwd: String::from("/home/user/work/repo"),
            sessions: vec![sample_session(
                "/tmp/s1.jsonl",
                "/home/user/work/repo",
                "abc123456789",
    ...
    fn remote_cli_ls_finds_absolute_folder_without_double_slash_path_bug() {
        ...
        app.config.machines.push(ConfigMachine {
            name: String::from("user@remote"),
            ssh_target: String::from("pi@192.0.2.124"),
            exec_prefix: None,
            codex_home: Some(String::from("/home/user/.codex")),
        });
        app.projects = vec![ProjectBucket {
            machine_name: String::from("user@remote"),
            machine_target: Some(String::from("pi@192.0.2.124")),
        ...
        let tree = app.cli_tree_lines(None).expect("tree");
        assert!(tree.iter().any(|line| line.contains("🖥 user@remote")));
        assert!(!tree.iter().any(|line| line.contains("//home/user")));
    ...
    fn group_project_actions_target_subtree_sessions() {
        ...
        app.projects = vec![
            ...
            ProjectBucket {
                ...
                machine_codex_home: None,
                machine_exec_prefix: None,
                cwd: String::from("/home/user/gh/repo-c"),
                sessions: vec![sample_session("/tmp/c.jsonl", "/home/user/gh/repo-c", "c")],
            },
        ];
    ...
    fn remote_session_path_uses_remote_codex_home_layout() {
        let ts = chrono::DateTime::parse_from_rfc3339("2026-03-14T17:00:00Z")
            ...
            .with_timezone(&Utc);
        assert_eq!(
            remote_session_dir("/home/user/.codex", ts),
            "/home/user/.codex/sessions/2026/03/14"
        );
        assert_eq!(
            remote_session_path("/home/user/.codex", ts, "session.jsonl"),
            "/home/user/.codex/sessions/2026/03/14/session.jsonl"
        );
    }
    ...
    fn normalize_group_cwd_handles_tree_style_paths() {
        ...
        assert_eq!(normalize_group_cwd("/"), "/");
        assert_eq!(normalize_group_cwd("//home/user"), "/home/user");
        assert_eq!(normalize_group_cwd("home/user/"), "/home/user");
    }
    ...
    fn group_prefix_target_collects_descendant_project_sessions() {
        ...
        app.projects = vec![
            ProjectBucket {
                ...
                machine_codex_home: Some(String::from("/home/user/.codex")),
                machine_exec_prefix: None,
                cwd: String::from("/home/user/work/app"),
                sessions: vec![sample_session("/tmp/a.jsonl", "/home/user/work/app", "a")],
            },
            ProjectBucket {
                machine_name: String::from("pi"),
                machine_target: Some(String::from("pi@192.0.2.124")),
                machine_codex_home: Some(String::from("/home/user/.codex")),
                machine_exec_prefix: None,
                cwd: String::from("/home/user/work/app/sub"),
                sessions: vec![sample_session("/tmp/b.jsonl", "/home/user/work/app/sub", "b")],
            },
        ];

        let targets = app
            .group_prefix_target("pi//home/user/work/app")
            .expect("targets");
        assert_eq!(targets.len(), 2);
        assert_eq!(targets[0].id, "a");
        assert_eq!(targets[1].id, "b");
    ...
    fn load_startup_local_state_repairs_stale_rows_outside_active_root() {
        ...
        let live_content = format!(
            concat!(
                r#"{{"timestamp":"2026-03-20T10:00:00Z","type":"session_meta","payload":{{"id":"{id}","timestamp":"2026-03-20T10:00:00Z","cwd":"/home/user/gh/yggdrasil","source":"cli","model_provider":"openai","cli_version":"0.0.0"}}}}"#,
                "\n",
        ...
        assert_eq!(rows[0].2, "/home/user/gh/yggdrasil");
    ...
    fn parse_config_machine_input_accepts_default_and_custom_codex_home() {
        let bare = parse_config_machine_input("pi@192.0.2.12").expect("bare");
        assert_eq!(bare.name, "192.0.2.12");
        assert_eq!(bare.ssh_target, "pi@192.0.2.12");
        assert_eq!(bare.codex_home, None);
        assert_eq!(bare.exec_prefix, None);

        let plain = parse_config_machine_input("pi=pi@192.0.2.12").expect("plain");
        assert_eq!(plain.name, "pi");
        assert_eq!(plain.ssh_target, "pi@192.0.2.12");
        assert_eq!(plain.codex_home, None);
        assert_eq!(plain.exec_prefix, None);

        let custom = parse_config_machine_input("lab=pi@192.0.2.13:/home/user/custom-codex")
            .expect("custom");
        assert_eq!(custom.name, "lab");
        assert_eq!(custom.ssh_target, "pi@192.0.2.13");
        assert_eq!(custom.codex_home.as_deref(), Some("/home/user/custom-codex"));
...
"#,
    ...
    fn scan_remote_machine_reuses_recent_cache() {
        ...
        let machine = ConfigMachine {
            name: String::from("pi"),
            ssh_target: String::from("pi@192.0.2.20"),
            exec_prefix: None,
            codex_home: Some(String::from("/home/user/.codex")),
        };
        let previous = RemoteMachineState {
            ...
            cached_projects: vec![ProjectBucket {
                machine_name: String::from("pi"),
                machine_target: Some(String::from("pi@192.0.2.20")),
                machine_codex_home: Some(String::from("/home/user/.codex")),
                machine_exec_prefix: None,
                cwd: String::from("/remote/repo"),
                sessions: vec![sample_session(
    ...
    fn paste_event_appends_to_input_prompt() {
        ...
        handle_paste_event(String::from("pi@192.0.2.124"), &mut app);

        assert_eq!(app.input, "pi=pi@192.0.2.124");
    }
    ...
    fn resolve_machine_target_supports_machine_prefixed_paths() {
        ...
        assert_eq!(target.name, "pi");
        assert_eq!(target.ssh_target.as_deref(), Some("pi@192.0.2.20"));
    ...
    fn project_rows_show_subtree_session_counts() {
        ...
        app.projects = vec![
            ProjectBucket {
                ...
                machine_codex_home: None,
                machine_exec_prefix: None,
                cwd: String::from("/home/user/gh"),
                sessions: vec![
                    sample_session("/tmp/gh-a.jsonl", "/home/user/gh", "abcdef0"),
                    sample_session("/tmp/gh-b.jsonl", "/home/user/gh", "abcdef1"),
                ],
            },
            ProjectBucket {
                machine_name: String::from("local"),
                ...
                machine_codex_home: None,
                machine_exec_prefix: None,
                cwd: String::from("/home/user/gh/codex-session-tui"),
                sessions: vec![
                    sample_session(
                        "/tmp/cst-a.jsonl",
                        "/home/user/gh/codex-session-tui",
                        "abcdef2",
                    ),
                    sample_session(
                        "/tmp/cst-b.jsonl",
        ...
        let gh_row = rows
            ...
            .find(|row| match row.kind {
                BrowserRowKind::Project { project_idx } => {
                    app.projects[project_idx].cwd == "/home/user/gh"
                }
                _ => false,
            })
    ...
    fn browser_tree_shows_all_sessions_under_project_leaf() {
        ...
        app.projects = vec![ProjectBucket {
            ...
            machine_codex_home: None,
            machine_exec_prefix: None,
            cwd: String::from("/home/user/gh/yggdrasil"),
            sessions: vec![
                sample_session(
                    "/tmp/one.jsonl",
                    "/home/user/gh/yggdrasil",
