...
- Added grouped-folder subtree remap behavior for typed folder rename/copy, which makes prefix rewrites like `/root -> /home/user` preserve child paths instead of flattening them.
...
- Fixed remote Browser tree path compression so absolute remote folders like `/home/user/data/cases` no longer render under a bogus `//home/user` node.
...
- Fixed browser path rendering to normalize accidental double-leading-slash paths such as `//home/user`.
